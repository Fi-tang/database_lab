...
int main(int argc, char *argv[])
{
    ...
    if(sscanf(data,"user=%[^&]&ord_id=%d&day=%d&train_id=%[^&]&train_id2=%[^&]&seat_type=%[^&]&seat_type2=%[^&]&start_time1=%d&arrive_time1=%d&start_time2=%d&arrive_time2=%d&data=%s",user,&ord_id,&day,train_id,train_id2,seat_type,seat_type2,&start_time1,
    &arrive_time1,&start_time2,&arrive_time2,data1)!=12){
        ...
        if(strcmp(seat_type2,"硬座")== 0){
            sprintf(string,"update seat \
                ...
                and se_station in ( \
                        ...
                        where pass_id = '%s' \
                        and pass_start > %d \
                        and pass_arrive <= %d)",train_id2, day,train_id2,start_time2,arrive_time2);
            res = PQexec(conn,string);
            PQclear(res);
        }
